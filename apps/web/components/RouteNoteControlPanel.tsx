...
export function RouteNoteControlPanel() {
  ...
  async function loadSnapshot() {
    ...
    try {
      ...
      dispatch({ type: "SNAPSHOT_LOADED", snapshot: data.snapshot as RouteNoteControlSnapshot });
    } catch (error) {
      const safe =
  ...
  async function prepareDraft() {
    ...
    try {
      ...
      const data = await responseJson(response);
      if (!response.ok || !data?.ok) throw readApiError(data);
      dispatch({ type: "DRAFT_READY", draft: data.draft as RouteNoteDraftSummary });
    } catch (error) {
      const safe =
        typeof error === "object" && error !== null && "code" in error
          ? (error as ApiError)
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <div>
        ...
        <h1 className="text-xl font-medium tracking-tight text-bone">RouteNote control</h1>
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-950/80 p-4 space-y-4">
        <div className="flex items-start justify-between gap-3">
          ...
          {state.hostAvailable ? (
            <span className="text-[10px] uppercase tracking-wider text-ash/40">Host ready</span>
          ) : (
            <span className="text-[10px] uppercase tracking-wider text-amber-400">Host unavailable</span>
          )}
        </div>
        ...
        <p className="text-[11px] text-ash/40">
          Connect RouteNote opens a private host desktop in a second tab for legitimate password, MFA, CAPTCHA, or account-verification steps. SongForge never receives those credentials.
        </p>
      </section>
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-950/80 p-4 space-y-4">
        <div>
          ...
          <select
            ...
            onChange={event => dispatch({ type: "RELEASE_SELECTED", releaseId: event.target.value })}
          ...
          >
            {state.releases.length === 0 ? <option value="">No releases available</option> : null}
        ...
        {selected ? (
          <div>
            <div className="flex items-center justify-between mb-1">
              <p className="text-[11px] uppercase tracking-[0.14em] text-ash/50">Readiness</p>
              <span
                className={cn(
        ...
        >
          {busy === "prepare" ? <Loader2 className="h-4 w-4 animate-spin" /> : <UploadCloud className="h-4 w-4" />}
          {busy === "prepare" ? "Preparing RouteNote draft" : "Prepare RouteNote Draft"}
        </button>

        {!connected ? (
          <p className="text-[11px] text-ash/40 text-center">Connect RouteNote before preparing a draft.</p>
        ) : selected && !selected.readiness.ready ? (
          <p className="text-[11px] text-ash/40 text-center">Complete the blocked release evidence before preparing this draft.</p>
        ) : null}
      </section>
        ...
        <section className="rounded-2xl border border-slate-800 bg-slate-950/80 p-4 space-y-3">
            ...
            <div className="space-y-1.5">
              {PENDING_STAGES.map((stage, index) => (
                <div key={stage} className="flex items-center gap-2 text-[12px] text-ash/50">
                  <span className="w-5 text-right text-[10px] text-ash/30">{String(index + 1).padStart(2, "0")}</span>
                  {stage}
                </div>
              ))}
            </div>
          ...
          {state.error ? (
            <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-3">
              <p className="text-[12px] font-medium text-amber-300">{state.error.message}</p>
              <p className="mt-1 text-[10px] uppercase tracking-wider text-ash/40">{state.error.code}</p>
            </div>
          ) : null}
        </section>
      )}
