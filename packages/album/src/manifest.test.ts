...
test("manifest blocks tracks 04-10 when source payload is missing", () => {
  const manifest = buildAlbumManifest({
    ...
    "04_PRETTY_BOY_PROBLEMS": {
      evidence_state: "BLOCKED_SOURCE_MISSING",
      documented_render_evidence: true
    },
...
test("manifest keeps visual assets non-applicable during audio-only foundation plan", () => {
  ...
  const cover = manifest.tracks[0]!.deliverables.find(
    (asset) => asset.filename === "ART/cover_3000x3000.png"
  );
